...
import { Loader2 } from "lucide-react"; // Make sure to install lucide-react or change to a basic spinner
...
export default function AuthPage() {
  ...
  const [isSubmitting, setIsSubmitting] = useState(false); // Fix for ghost buttons
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (isSubmitting) return; // Prevent double submission trigger
    ...
    try {
      if (isLogin) {
        ...
        localStorage.setItem("token", res.data.token);
        alert("Signup success. Please check your email for the verification code.");
        setIsOtpStage(true);
      }
    } catch (err) {
      console.log(err);
          ...
          <div className="w-full max-w-md bg-white/5 border border-white/10 backdrop-blur-xl rounded-3xl p-8 shadow-2xl">
            {isOtpStage ? (
              <>
                <div className="text-center mb-8">
                  ...
                  <p className="text-gray-400 text-sm">
                    Enter the 6-digit confirmation code dispatched cleanly to your email address.
                  </p>
                </div>

                <form onSubmit={handleOtpSubmit} className="space-y-5">
                  <div>
                    <label className="text-xs font-semibold text-gray-400 uppercase ml-1">Verification Code</label>
                    <input
                      type="text"
                      maxLength="6"
                      ...
                      placeholder="000000"
                      className="w-full mt-1.5 px-4 py-3 bg-white/5 border border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 text-center text-xl font-bold tracking-[8px] transition placeholder:text-gray-600 placeholder:tracking-normal disabled:opacity-50"
                    />
                  </div>
                  ...
                  <button
                    type="submit"
                    disabled={isSubmitting}
                    className="w-full bg-emerald-600 hover:bg-emerald-500 text-white py-3 rounded-xl font-semibold transition-all transform active:scale-[0.98] shadow-lg shadow-emerald-600/20 mt-2 flex justify-center items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
              ...
              <>
                ...
                <button disabled={isSubmitting} className="w-full flex items-center justify-center gap-3 bg-white/10 hover:bg-white/15 border border-white/10 py-2.5 rounded-xl text-sm font-medium transition mb-6 disabled:opacity-50">
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  {!isLogin && (
                    <div>
                      ...
                      <input
                        ...
                        className="w-full mt-1.5 px-4 py-3 bg-white/5 border border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition placeholder:text-gray-600 disabled:opacity-50"
                      />
                    </div>
                  )}
                  ...
                  <button
                    type="submit"
                    disabled={isSubmitting}
                    className="w-full bg-indigo-600 hover:bg-indigo-500 text-white py-3 rounded-xl font-semibold transition-all transform active:scale-[0.98] shadow-lg shadow-indigo-600/20 mt-2 flex justify-center items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {isSubmitting && <Loader2 size={16} className="animate-spin" />}
                ...
                <p className="text-sm text-gray-400 text-center mt-8">
                  ...
                  <button
                    ...
                    className="ml-2 text-indigo-400 font-semibold hover:text-indigo-300 transition underline underline-offset-4 disabled:opacity-50"
                  >
